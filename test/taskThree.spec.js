...
describe("Events Endpoints", function () {
	...
	describe("GET all taskThree", () => {
		context(`Given no taskThree`, () => {
			beforeEach("insert taskThree", () => {
				return db
					.into("users")
					.insert(testUsers)
					.then(() => {
			...
			it(`responds with 200 and an empty list`, () => {
				return supertest(app)
					.post("/api/taskThree/getAll")
					.set(
						"Authorization",
						`bearer ${AuthService.createJwt(testUser.user_email, { user_id: testUser.id })}`
					)
					.send({ project_id: testProjects[0].id })
					.expect(200, []);
			});
		});

		context("Given there are taskThree in the database", () => {
			beforeEach("insert taskThree", () => {
				return db
					.into("users")
					.insert(testUsers)
			...
			it("responds with 200 and all of the taskThree", () => {
				return supertest(app)
					.post("/api/taskThree/getAll")
					.set(
						"Authorization",
