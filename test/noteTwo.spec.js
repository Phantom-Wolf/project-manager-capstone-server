...
describe("Events Endpoints", function () {
	...
	describe("GET all noteTwo", () => {
		context(`Given no noteTwo`, () => {
			beforeEach("insert noteTwo", () => {
				return db
					.into("users")
					.insert(testUsers)
					.then(() => {
			...
			it(`responds with 200 and an empty list`, () => {
				return supertest(app)
					.post("/api/noteTwo/getAll")
					.set(
						"Authorization",
						`bearer ${AuthService.createJwt(testUser.user_email, { user_id: testUser.id })}`
					)
					.send({ parent_id: testTasks2[0].id })
					.expect(200, []);
			});
		});

		context("Given there are noteTwo in the database", () => {
			beforeEach("insert noteTwo", () => {
				return db
					.into("users")
					.insert(testUsers)
					...
					.then(() => {
						return db.into("notetwo").insert(testNotes2);
					});
			});

			it("responds with 200 and all of the noteTwo", () => {
				return supertest(app)
					.post("/api/noteTwo/getAll")
					.set(
						"Authorization",
