...
describe("Events Endpoints", function () {
	...
	describe("GET all taskTwo", () => {
		context(`Given no taskTwo`, () => {
			beforeEach("insert taskTwo", () => {
				return db
					.into("users")
					.insert(testUsers)
					.then(() => {
		...
		context("Given there are taskTwo in the database", () => {
			beforeEach("insert taskTwo", () => {
				return db
					.into("users")
					.insert(testUsers)
			...
			it("responds with 200 and all of the taskTwo", () => {
				return supertest(app)
					.post("/api/taskTwo/getAll")
					.set(
						"Authorization",
