...
describe("Events Endpoints", function () {
	...
	describe("GET all parentNotes", () => {
		context(`Given no parentNotes`, () => {
			beforeEach("insert parentTasks", () => {
				return db
					.into("users")
					.insert(testUsers)
					.then(() => {
			...
			it(`responds with 200 and an empty list`, () => {
				return supertest(app)
					.post("/api/parentNote/getAll")
					.set(
						"Authorization",
						`bearer ${AuthService.createJwt(testUser.user_email, { user_id: testUser.id })}`
					)
					.send({ parent_id: testTasks0[0].id })
					.expect(200, []);
			});
		});

		context("Given there are parentNotes in the database", () => {
			beforeEach("insert parentNotes", () => {
				return db
					.into("users")
					.insert(testUsers)
					.then(() => {
					...
					.then(() => {
						return db.into("parentnote").insert(testNotes0);
					});
			});

			it("responds with 200 and all of the parentNotes", () => {
				return supertest(app)
					.post("/api/parentNote/getAll")
					.set(
						"Authorization",
