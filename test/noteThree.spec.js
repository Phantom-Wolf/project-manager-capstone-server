...
describe("Events Endpoints", function () {
	...
	describe("GET all noteThree", () => {
		context(`Given no noteThree`, () => {
			beforeEach("insert noteThree", () => {
				return db
					.into("users")
					.insert(testUsers)
					.then(() => {
					...
					.then(() => {
						return db.into("tasktwo").insert(testTasks2);
					})
					.then(() => {
						return db.into("taskthree").insert(testTasks3);
					});
			});

			it(`responds with 200 and an empty list`, () => {
				return supertest(app)
					.post("/api/noteThree/getAll")
					.set(
						"Authorization",
						`bearer ${AuthService.createJwt(testUser.user_email, { user_id: testUser.id })}`
					)
					.send({ parent_id: testTasks3[0].id })
					.expect(200, []);
			});
		});

		context("Given there are noteThree in the database", () => {
			beforeEach("insert noteThree", () => {
				return db
					.into("users")
					.insert(testUsers)
					...
					.then(() => {
						return db.into("notethree").insert(testNotes3);
					});
			});

			it("responds with 200 and all of the noteThree", () => {
				return supertest(app)
					.post("/api/noteThree/getAll")
					.set(
						"Authorization",
