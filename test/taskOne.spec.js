...
describe("Events Endpoints", function () {
	...
	describe("GET all taskOne", () => {
		context(`Given no taskOne`, () => {
			beforeEach("insert taskOne", () => {
				return db
					.into("users")
					.insert(testUsers)
					.then(() => {
		...
		context("Given there are taskOne in the database", () => {
			beforeEach("insert taskOne", () => {
				return db
					.into("users")
					.insert(testUsers)
			...
			it("responds with 200 and all of the taskOne", () => {
				return supertest(app)
					.post("/api/taskOne/getAll")
					.set(
						"Authorization",
