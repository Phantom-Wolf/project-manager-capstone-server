...
const TaskThreeService = require("./taskThree-service");
const { json } = require("express");
const path = require("path");

// middleware

const taskThreeRouter = express.Router();
...
taskThreeRouter
	...
	.get((req, res, next) => {
		...
		TaskThreeService.getAllTasks(req.app.get("db"), project_id).then((tasks) => {
	...
	.post(requireAuth, jsonParser, (req, res, next) => {
		...
		TaskThreeService.insertTask(req.app.get("db"), newTask)
			.then((task) => {
				res
					.status(201)
...
taskThreeRouter.route("/getAll").post(requireAuth, jsonParser, (req, res, next) => {
	const { project_id } = req.body;
...
taskThreeRouter
	...
	.all((req, res, next) => {
		...
		TaskThreeService.getTaskById(req.app.get("db"), req.params.task_id)
			.then((task) => {
				if (!task) {
					return res.status(404).json({
	...
	.delete((req, res, next) => {
		TaskThreeService.deleteTask(req.app.get("db"), req.params.task_id)
	...
	.patch(jsonParser, (req, res, next) => {
		...
		TaskThreeService.updateTask(req.app.get("db"), req.params.task_id, taskToUpdate)
			.then((updatedTask) => {
				res.status(200).json();
			})
			.catch(next);
...
taskThreeRouter
	...
	.get((req, res, next) => {
		...
		TaskThreeService.getAllNotes(req.app.get("db"), parent_id).then((Note) => {
	...
	.post(requireAuth, jsonParser, (req, res, next) => {
		...
		TaskThreeService.insertNote(req.app.get("db"), newNote)
			.then((Note) => {
				res
					.status(201)
...
taskThreeRouter
	...
	.all((req, res, next) => {
		...
		TaskThreeService.getNoteById(req.app.get("db"), req.params.note_id)
			.then((Note) => {
				if (!Note) {
					return res.status(404).json({
	...
	.delete((req, res, next) => {
		TaskThreeService.deleteNote(req.app.get("db"), req.params.note_id)
			.then((numbRowsAffected) => {
				res.status(204).end();
			})
			.catch(next);
...
module.exports = taskThreeRouter;
